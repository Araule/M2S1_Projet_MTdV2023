...
def check_operation(tokens, types, line_n, line):
	...
	TODO
	ATTENTION : appel de cette fonction uniquement sur la partie droite ou la partie gauche
	sous forme de liste de tokens ET liste de types
	
	en entrée je ne dois avoir QUE:
	variable_regex = r"([a-z]|[A-Z]|_)([a-z]|[A-Z]|[0-9]|_)*"
	chiffre_regex = r"[0-9]+"
	operateur_regex = r"[\+\*]"
	ex : ['x', '0', 'x',  'x', '+', '1', 'y',  x * 2 ', 'boucle ', 'si (x == 2) ', 'fin } ', 'x = 2 ', '} ', 'I ', '# ']
	...
	vérifier que le format est bien comme ci-dessus
	...
	for token, typen in zip(tokens, types):
		...
		if token == tokens[-1] and i%2 != 0 and (re.search(variable_regex, token) or re.search(chiffre_regex, token)):
			print(f"dernier token : {token}")
		elif token == tokens[-1] and i%2 == 0 and re.search(operateur_regex, token):
			print(f"le dernier token est un opérateur : {token} il manque une variable ou une valeur")
			erreur("Opération invalide", token = tokens[-1], line_n = i, line = line)
		elif i%2 != 0 and (re.search(variable_regex, token) or re.search(chiffre_regex, token)):
			print(f"impair : {token} et : {typen}")
		elif i%2 == 0 and re.search(operateur_regex, token):
			print(f"pair : {token} et : {typen}")
		else:
			erreur("Opération invalide", token = tokens[i], line_n = i, line = line)
